...
export default function AddChildScreen() {
    ...
    const handleFormSubmit = async (data: any) => {
        ...
        if (image) {
            await api
                .children()
                .uploadImage(createFormData(image, { childId: childId.data }));
        }

        await api.schools().addChildToSchool(school._id, childId.data);
        await api.reports().createNewReport(childId.data);
        await api.schools().addNewEvent(school._id, {
            details: "",
            startTime: data.childBirthDate,
            title: `יום הולדת ל${data.childFirstName} ${data.childLastName}`,
        });
